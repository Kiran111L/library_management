...
def login():
    if request.method == 'POST':
        ...
        cursor.execute(f"SELECT * FROM {role.capitalize()} WHERE username=%s AND password=%s", (username, password))
...
def manage_books():
    ...
    cursor_dict.execute("""SELECT Book.id, Book.title, Author.name AS author, Publisher.name AS publisher, Book.quantity 
                           FROM Book 
                           JOIN Author ON Book.author_id = Author.id 
                           JOIN Publisher ON Book.publisher_id = Publisher.id""")
    books = cursor_dict.fetchall()
    cursor_dict.close()
